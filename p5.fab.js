...
p5.prototype.registerMethod('init', function fabInit() {
  /**

   */
  // let data = loadJSON('machines/ender3.json');
  // console.log(data);
});
...
class Fab {
  ...
  */
  ...
  moveRetract(x, y, z, v = 25, e = 8) {
    ...
    // prime the nozzle
    var primeCmd = `G1 E${e} F4500 ;prime`;// removing comment for prusa ;prime`;
    this.add(primeCmd);
  ...
  saveGcode(fname) {
    let writer = createWriter(fname + ".gcode");
    let gc = "";
    // prusa start gcode
    gc += `M73 P0 R10
    M73 Q0 S10
